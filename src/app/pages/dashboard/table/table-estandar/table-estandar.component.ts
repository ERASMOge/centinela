...
@Component({
  selector: 'app-table-estandar',
  templateUrl: './table-estandar.component.html',
  styleUrls: ['./table-estandar.component.css']
})
export class TableEstandarComponent implements OnInit {
  ...
  @Input() estandar : number = 0

  @Input() dialogRef? :  MatDialogRef<ViewDentroEstandarComponent | ViewFueraEstandarComponent>
  ...
  async cargarTabla(){
    ...
    await this.serviceDash.cargarDentroFueraEstandar(this.selectedFake,this.selectedFake2,this.estatus,this.auth.getCveGrupo(), this.estandar).subscribe( (resp:any) =>{
